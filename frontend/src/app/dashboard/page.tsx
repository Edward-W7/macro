...
export default function Dashboard() {
  ...
  // LockIcon is now imported from standalone file
  // State to show/hide all filters
  const [filtersExpanded, setFiltersExpanded] = useState(true);

  // load json dropdown options
  ...
  function handleDropdownChange(
    ...
    setExclusions(prev => {
      ...
      setMealData([]); // clear shit on a filter change cuz they no longer apply
      return updated;
    });
  }
                        ...
                        <tr key={sortedIdx} style={{ borderBottom: '1px solid #3b3b4f', minHeight: '56px', height: '56px', verticalAlign: 'middle' }}>
                          <td style={{ padding: '0.7rem', fontWeight: 600, verticalAlign: 'middle', height: '56px', display: 'flex', alignItems: 'center' }}>
                            <span
                              ...
                              style={{ display: 'flex', alignItems: 'center', marginRight: '0.7rem' }}
                            >
                              <LockIcon locked={locked} />
                            </span>
                            <span style={{ flex: 1, textAlign: 'center', width: '100%' }}>
                              {typeof meal.dish_name === 'string' ? meal.dish_name.trim() : 'Unnamed Dish'}
                            </span>
                          </td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof mealTime === 'string' ? mealTime.trim() : mealTime}</td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof meal.restaurant === 'string' ? meal.restaurant.trim() : 'Unknown'}</td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof meal.calories === 'string' ? meal.calories.trim() : meal.calories}</td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof meal.macros.protein === 'string' ? meal.macros.protein.trim() : meal.macros.protein}g</td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof meal.macros.carbohydrates === 'string' ? meal.macros.carbohydrates.trim() : meal.macros.carbohydrates}g</td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof meal.macros.fat === 'string' ? meal.macros.fat.trim() : meal.macros.fat}g</td>
                          <td style={{ padding: '0.7rem', verticalAlign: 'middle', height: '56px', textAlign: 'center' }}>{typeof meal.notes === 'string' ? meal.notes.trim() : ''}</td>
                        </tr>
                      );
                    });
                  })()}
